package Question8
import scala.collection.parallel._
object MainApp  extends App {
  val digits : String= ""


  //println(rollingHash(digits,4) + "rolling")
  //println(vanilla(digits,13)+ "vanilla")
  println(multithread(digits,13))
  def rollingHash (digits : String, productDigits : Int) : Double = {
    var current : Double = digits.slice(0, productDigits).toCharArray.map(_.toString.toInt).reduce(_ * _)
    var biggest : Double = current
    for (d <- (productDigits until digits.length)) {
      if (!(digits.charAt(d - productDigits).toString.toDouble !=0 || digits.charAt(d).toString.toDouble ==0)) {
        current = current * digits.charAt((d)).toString.toDouble / digits.charAt(d - (productDigits)).toString.toDouble
        if (current >= biggest) {
          biggest = current
        }
      }
    }
    return biggest
  }

  def vanilla (digits : String, productDigits : Int) : Long = {
    var biggest : Long = digits.slice(0, productDigits).toCharArray.map(_.toString.toInt).reduce(_ * _)
    for (d <- (0 until digits.length)) {
      val current: Long = digits.slice(d, d+ productDigits).toCharArray.map(_.toString.toLong).product
      //val current: Long = digits.slice(d, d+ productDigits).toCharArray.map(_.toString.toInt).reduce(_ * _)
      if (current >= biggest) {
        biggest = current
      }
    }
    return biggest
  }

  def multithread(digits : String, productDigits : Int) : Long = {
    val subdigits: Array[String] = digits.split('0').filter(_.size > productDigits)
    return subdigits.map(x => vanilla(x, productDigits)).max
  }

}
